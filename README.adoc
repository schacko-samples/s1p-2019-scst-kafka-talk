...
Supplier produces some data every second (the default schedule) to a Kafka topic and then the consumer receives it from that topic.
The definition for this flow is supplier;consumer.
Take a look at the application.properties to see how this is defined.
There is another flow which is parallel to the first one.
The difference is that this is a composed flow - supplier|function.
The function definition in the configuration makes these two completely isolated flows.
The composed one starts with the same supplier as the first one where it produces every second, but then it passes through a function where the data is transformed.
The link between the supplier and the function in this case is in-memory, i.e. the only time the data is sent to Kafka it on the output of the function.
Finally, we have one more isolated definition for a consumer where it consumes from the topic where the composed flow sent the data.

In order to run the application, simply run it, and you will see the data getting logged on the console.
...
Following commands are issued using `kafkacat`.
You can also use other tools like the Kafka console producer/consumer scripts.
...
Enter some text through the `words` topic and watch the counts getting updated in the `counts` topic.
...
Run the app `SpringCloudStreamApp4Application`, then the producer app - `UserClicksRegionProducerApplication`.
